# WRITE  BY @F_O_1
# PLUGIN FOR robin
# @HELLASUserBot

import random
from telethon import events
import random, re

from JoKeRUB.utils import admin_cmd

import asyncio
from JoKeRUB import l313l

from ..core.managers import edit_or_reply
from hellas import ch_t2ag, ch_tag
plugin_category = "extra"

# اوامر الادمن لسورس Robin

from hellas import CH  # استدعاء المتغير CH من ملف hellas.py

@l313l.ar_cmd(
    pattern="اوامر الحظر$",
    command=("اوامر الحظر", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الحـظر \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"( `.حظر` ) \n-  تقوم بالرد على شخص او وضع معرفه مع الامر وسيحظره من المجموعة\n\n"
            f"( `.الغاء حظر` )\n - بالرد على الشخص او كتابة معرفه مع الامر لالغاء حظره\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"⌔︙CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الكتم$",
    command=("اوامر الكتم", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الكـتم \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"( `.كتم` ) \n-  تقوم بالرد على شخص او وضع معرفه مع الامر وسيكتمه من المجموعة\n\n"
            f"( `.الغاء كتم` )\n - بالرد على الشخص او كتابة معرفه مع الامر لالغاء كتمه\n\n"
            f"( `.كتم_مؤقت+عدد الساعات او الدقائق+السبب` )\n"
            f"- بالرد على الشخص او كتابة معرفه مع الامر لتقيده من المجموعة مؤقتا\n"
            f"**مثال : كتم_مؤقت 1h مخالف القوانين**\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"⌔︙CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التثبيت$",
    command=("اوامر التثبيت", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر التثبيت \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"( `.تثبيت` ) \n-  تقوم بالرد على الرسالة مع الامر وستثبت في المجموعة\n\n"
            f"( `.الغاء التثبيت` )\n - بالرد على الرسالة مع الامر لإلغاء تثبيتها\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"⌔︙CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الاشراف$",
    command=("اوامر الاشراف", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الاشراف \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"( `.رفع مشرف` ) \n-  تقوم بالرد على الشخص مع الامر و سيرفع مشرفا في المجموعة\n\n"
            f"( `.تك` )\n - بالرد على الشخص مع الامر لإنزاله من الاشراف في المجموعة\n\n"
            f"( `.ارفع` ) \n- لرفع المستخدم في جميع المجموعات مع كافة الصلاحيات مع لقب مخفي\n\n"
            f"( `.نزل` ) \n-لتنزيل الشخص من رتبة الاشراف في جميع المجموعات\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙CH : {CH}"
        )


from hellas import CH  # استدعاء المتغير CH من ملف hellas.py

# اوامر التفليش
@l313l.ar_cmd(
    pattern="اوامر التفليش$",
    command=("اوامر التفليش", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر التفليـش \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.تفليش بالطرد` ) \n ارسل الامر لطرد جميع الاعضاء من المجموعه \n\n"
            f"- ( `.تفليش` ) \n كتابة الامر فقط في المجموعه لحظر جميع الاعضاء\n\n"
            f"- ( `.حظر_الكل` ) \n كتابة الامر فقط في المجموعه لحظر جميع الاعضاء بدون صلاحيات عن طريق بوت الحماية \n\n"
            f"- ( `.طرد_الكل` ) \n كتابة الامر فقط في المجموعه ليقوم بطرد جميع الاعضا بدون صلاحيات عن طريق بوت الحماية \n\n"
            f"- ( `.كتم_الكل` ) \n كتابة الامر فقط في المجموعه ليقوم بكتم جميع الاعضاء بدون صلاحيات اشراف عن طريق بوت الحماية\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

# اوامر المحذوفين
@l313l.ar_cmd(
    pattern="اوامر المحذوفين$",
    command=("اوامر المحذوفين", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر المـحذوفين \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.حذف المحظورين` ) \n كتابة الامر في الكروب لالغاء حظر جميع الاعضاء \n\n"
            f"- ( `.اطردني` ) \n فقط ارسل الامر في المجموعة لمغادرة المجموعه \n\n"
            f"- ( `.المحذوفين` ) \n لعرض الحسابات المحذوفة في مجمـوعة معينة ولحذفهم ارسل .المحذوفين اطردهم\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

# اوامر الكروب
@l313l.ar_cmd(
    pattern="اوامر الكروب$",
    command=("اوامر الكروب", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الكروب \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.الاحداث` ) \n كتابة الامر في الكروب لعرض احداث الكروب\n\n"
            f"- ( `.الاعضاء` ) \n فقط ارسل الامر في المجموعة لعرض اعضاء المجموعة\n\n"
            f"- ( `.المشرفين` ) \n ارسل الامر في المجموعه لعرض حسابات المشرفين\n\n"
            f"- ( `.البوتات` )\n ارسل الامر في المجموعه لعرض البوتات\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

# اوامر الترحيب
@l313l.ar_cmd(
    pattern="اوامر الترحيب$",
    command=("اوامر الترحيب", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الترحيب \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"--( `.ترحيب + ترحيبك` )\n اكتب الامر مع ترحيب في المجموعه ليرحب بالاعضاء الجدد\n\n"
            f"- ( `.حذف الترحيب` ) \n فقط ارسل الامر في المجموعة لحذف الترحيبات\n\n"
            f"- ( `.الترحيب` )\n ارسل الامر في المجموعه لعرض ترحيبات المجموعة\n\n"
            f"- ( `.الترحيب السابق ايقاف/تشغيل` )\n لتعطيل اخر ترحيب وضعته في المجموعة او تشغيل\n\n"
            f"- ( `.رحب + ترحيبك` )\n لوضع ترحيب عند دخول الاعضاء للمجموعة في الخاص\n\n"
            f"- ( `.حذف رحب` )\n لحذف الترحيب في الخاص\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

# اوامر الردود
@l313l.ar_cmd(
    pattern="اوامر الردود$",
    command=("اوامر الردود", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الـردود \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.رد + ردك` ) \n لوضع رد معين في المجموعة اكتب الامر وردك\n\n"
            f"- ( `.حذف الردود` ) \n فقط ارسل الامر في المجموعة لحذف الردود المضافة\n\n"
            f"- ( `.الردود` ) \n ارسل الامر في المجموعه لعرض ردود المجموعة\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الحماية$",
    command=("اوامر الحماية", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الـخاص \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.الحماية تشغيل/تعطيل` ) \n لتشغيل امر الحمايه او تعطيله في الـخاص \n"
            f"- ( `.سماح` ) \n بالرد على الشخص للسماح له بالتكلم في الخاص\n"
            f"- ( `.رفض` ) \n بالرد على الشخص لرفضه من الخاص \n"
            f"- ( `.المسموح لهم` )\n فقط ارسل الامر لاظهار الاشخاص المسموح لهم والمرفوضين\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التلكراف$",
    command=("اوامر التلكراف", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر التلكراف \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.تلكراف ميديا` ) \n لاستخراج رابط من الصورة على شكل رابط تلكراف  \n\n"
            f"- ( `.تلكراف نص` ) \n بالرد على النص او المقالة لصنع رابط تلكراف للنص\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الانتحال$",
    command=("اوامر الانتحال", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الانتحال \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.انتحال` ) \n بالرد على الشخص لنسخ حسابه بالكامل من صورة واسم وبايو  \n\n"
            f"- ( `.اعادة` ) \n لارجاع الحساب الى وضعه الطبيعي لما كان سابقا\n"
            f"- ( `.انتحال_الدردشه` ) \n قم بكتابة الامر مع معرف المجموعة او القناة بدون @\n"
            f"- ( `.اعادة_الدردشه` ) \n لارجاع الدردشه الى وضعه الطبيعي لما كان سابقا\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التقليد$",
    command=("اوامر التقليد", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الـتقليـد \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.تقليد` ) \n بالرد على الشخص لتقليد جميع رسائله في الدردشه \n\n"
            f"- ( `.الغاء التقليد` ) \n بالرد على الشخص لايقاف التقليد\n"
            f"- ( `.المقلدهم` ) \n لاظهـار قائمه الاشخاص الذي فعـلت عليهم امر التقليد ولمسحهم ارسل  (`.مسح المقلدهم`) \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر المنشن$",
    command=("اوامر المنشن", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الـمنشن \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- (`.منشن + المسج`)\n قم بكتابة الامر في المجموعة لعمل تاك مفرد للاعظاء الموجودين\n"
            f"- (`.الغاء منشن`)\n لألغاء التاك \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التحميل$",
    command=("اوامر التحميل", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن امـر التحمـيل\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر\n\n"
            f"- (`.بحث + اسم الاغنية`)\n بكتابة اسم الاغنية مع الامر لارسال الاغنيه مباشرا واذا ما اشتغل جرب اوامر التحميل الاخرى\n- مثـال : `.بحث ماهر زين`\n\n"
            f"- (`.يوت + اسم الفيديو او الاغنية`)\n كتابة الامر مع اسم الفيديو او الاغنية ليعطيك نتائج البحث وروابط من يوتيوب تستخدم مع اوامر التحميل\n- مثـال : `.يوت ماهر زين`\n\n"
            f"- (`.تحميل ص + رابط الاغنية`)\n لتحميل اغنيه من خلال وضع الرابط مع الامر\n- مثـال : `.تحميل ص https://youtube.com/...`\n\n"
            f"- (`.فيديو + اسم الاغنية`)\n كتابة الامر مع اسم المقطع لتحميله وارساله\n- مثـال : `.فيديو باسم الكربلائي`\n\n"
            f"- (`.انستا + الرابط `)\n يستخدم هذا الامر لتحميل من الانستا فقط اكتب الامر مع رابط الفيديو ليحمله\n- مثـال : `.انستا https://instagram.com/jdisjejjd...`\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الترجمة$",
    command=("اوامر الترجمة", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الترجمة \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.ترجمة ar` ) \n بالرد على النص لترجمته للغه  ية\n\n"
            f"- ( `.ترجمة en` ) \n بالرد على النص لترجمته للغه الانكليزية\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر النطق$",
    command=("اوامر النطق", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الـنطق \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.تكلم ar` ) \n بالرد على النص لتحويله الى مقطع صوتي للغة  يه \n\n"
            f"- ( `.تكلم en` ) \n بالرد على النص لتحوليه الى مقطع صوتي للغه الانكليزية\n\n"
            f"- ( `.احجي ar` ) \n بالرد على مقطع صوتي او بصمة لتحوليه الى نص للغه  ية\n\n"
            f"- ( `.احجي en` ) \n بالرد على مقطع صوتي او بصمة لتحوليه الى نص للغه الانكليزية\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر القفل$",
    command=("اوامر القفل", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر القـفـل \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.قفل + الاضافة` ) \n تكتب الامر مع الاضافة لقفل شي معين في المجموعة \n\n"
            f"الاضافات  : \n"
            f"- الدردشه  : لقفل ارسال الرسائل \n"
            f"- الوسائط   : لقفل ارسال الوسائط\n"
            f"- الملصقات  : لقفل ارسال الملصقات\n"
            f"- الروابط  : لقفل ارسال الروابط\n"
            f"- المتحركه  : لقفل ارسال المتحركه\n"
            f"- الالعاب  : لقفل ارسال الالعاب الانلاين\n"
            f"- الانلاين  : لقفل ارسال البوتات الانلاين\n"
            f"- التصويت  : لقفل ارسال التوصيتات \n"
            f"- الكل :  لقفل ارسال كل شي\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الفتح$",
    command=("اوامر الفتح", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الفـتـح \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.فتح + الاضافة` ) \n تكتب الامر مع الاضافة لفتـح شي معين في المجموعة \n\n"
            f"الاضافات  : \n"
            f"- الدردشه  : لفتح ارسال الرسائل \n"
            f"- الوسائط   : لفتح ارسال الوسائط\n"
            f"- الملصقات  : لفتح ارسال الملصقات\n"
            f"- الروابط  : لفتح ارسال الروابط\n"
            f"- المتحركه  : لفتح ارسال المتحركه\n"
            f"- الالعاب  : لفتح ارسال الالعاب الانلاين\n"
            f"- الانلاين  : لفتح ارسال البوتات الانلاين\n"
            f"- التصويت  : لفتح ارسال التوصيتات \n"
            f"- الكل :  لفتح ارسال كل شي\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر المنع$",
    command=("اوامر المنع", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"شـرح عـن اوامـر الـمنـع \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ اختر احدى هذه الاوامر \n\n"
            f"- ( `.منع + الكلمة` ) \n لمـنع الـكلمة في الـدردشة وسيتم حذفها عند ارسالها من اي شخص \n\n"
            f"- ( `.الغاء منع + الكلمة` ) \n لالغاء منع الكلمة والسماح للجميع بأرسالها في الدردشة\n\n"
            f"- ( `.قائمة المنع` ) \nلاظهـار قائمه الكـلمات الـتي منعـتها في الـدردشـه \n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            f"᯽︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 1$",
    command=("تسلية 1", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"قائمة اوامر التسـلية 1 :\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"᯽︙ اختر احدى هذه القوائم\n\n"
            f"- ( `.غبي` )\n- ( `.القنابل` )\n- ( `.اتصل` )\n- ( `.قتل` )\n- ( `.شنو` )\n- ( `.طوبة` )\n- ( `.مربعات` )\n- ( `.حلويات` )\n- ( `.نار` )\n- ( `.شحن` )\n\n"
            f"تحذيـر : لا تكثر من استخدام هذه الاوامر لانها قد تسبب ضغط على حسابك او تعليقه من ارسال الرسائل لفترة وجيزة.\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 2$",
    command=("تسلية 2", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"قائمة اوامر التسـلية 2 :\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"᯽︙ اختر احدى هذه القوائم\n\n"
            f"- ( `.افكر` )\n- ( `.متت` )\n- ( `.ضايج` )\n- ( `.ساعه` )\n- ( `.مح` )\n- ( `.قلب` )\n- ( `.جيم` )\n- ( `.الارض` )\n- ( `.قمر` )\n- ( `.اقمار` )\n- ( `.قمور` )\n\n"
            f"تحذيـر : لا تكثر من استخدام هذه الاوامر لانها قد تسبب ضغط على حسابك او تعليقه من ارسال الرسائل لفترة وجيزة.\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 3$",
    command=("تسلية 3", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"قائمة اوامر التسـلية 3 :\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"᯽︙ اختر احدى هذه القوائم\n\n"
            f"- ( `.نجمه` )\n- ( `.مكعبات` )\n- ( `.مطر` )\n- ( `.تفريغ` )\n- ( `.فليم` )\n- ( `.احبك` )\n- ( `.طائره` )\n- ( `.شرطه` )\n- ( `.النضام الشمسي` )\n\n"
            f"تحذيـر : لا تكثر من استخدام هذه الاوامر لانها قد تسبب ضغط على حسابك او تعليقه من ارسال الرسائل لفترة وجيزة.\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 4$",
    command=("تسلية 4", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"قائمة اوامر التسـلية 4 :\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"᯽︙ اختر احدى هذه القوائم\n\n"
            f"- ( `.قاتل` )\n- ( `.عين` )\n- ( `.افكرر` )\n- ( `.افعى` )\n- ( `.رجل` )\n- ( `.مايكرو` )\n- ( `.فايروس` )\n- ( `.قطار` )\n- ( `.نيكول` )\n- ( `.موسيقى` )\n- ( `.رسم` )\n\n"
            f"تحذيـر : لا تكثر من استخدام هذه الاوامر لانها قد تسبب ضغط على حسابك او تعليقه من ارسال الرسائل لفترة وجيزة.\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 5$",
    command=("تسلية 5", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"قائمة اوامر التسـلية 5 :\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"᯽︙ اختر احدى هذه القوائم\n\n"
            f"- ( `.تحميل` )\n- ( `.مربع` )\n- ( `.دائره` )\n- ( `.انيم` )\n- ( `.بشره` )\n- ( `.قرد` )\n- ( `.يد` )\n- ( `.العد التنازلي` )\n- ( `.قلوب` )\n\n"
            f"تحذيـر : لا تكثر من استخدام هذه الاوامر لانها قد تسبب ضغط على حسابك او تعليقه من ارسال الرسائل لفترة وجيزة.\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 6$",
    command=("تسلية 6", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"قائمة اوامر التسـلية 6 :\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"᯽︙ اختر احدى هذه القوائم\n\n"
            f"- ( `.ترامب` + النص )\n- ( `.مودي` + النص )\n- ( `.بنر` + النص )\n- ( `.كانا` + النص )\n- ( `.تويت` + المعرف ; النص )\n\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 7$",
    command=("تسلية 7", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            f"قائمة اوامر التسـلية 7 :\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"᯽︙ اختر احدى هذه القوائم\n\n"
            f"- ( `.تراش` بالرد على ملصق/صورة )\n- ( `.تهديد` بالرد على ملصق/صورة )\n- ( `.فخ`  بالرد على ملصق/صورة + الاسم1 ; الاسم2 )\n- ( `.بورن`  بالرد على ملصق/صورة + المعرف ; النص )\n- ( `.تويت` + المعرف ; النص )\n\n"
            f"★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            f"⌔︙ CH : {CH}"
        )

@l313l.ar_cmd(
    pattern="تسلية 8$",
    command=("تسلية 8", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر التسـلية 8 :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه القوائم\n\n"
            "- ( `.نص` + النص )\n- ( `.ستيكر` + النص )\n- ( `.هونك` + النص )\n- ( `.تغريد` + النص )\n- ( `.غلاكس` + النص )\n- ( `.دوغي` + النص )\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الملصقات$",
    command=("اوامر الملصقات", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شـرح عـن اوامـر الملصقـات\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.ملصق` )\nبالرد على الملصق لأخذه و عمل حزمه خاصة بك و اضافته بها\n\n"
            "- ( `.حزمة` )\nبالرد على الملصق لنسخ الحزمة كاملة بداخل حزمة ملصقاتك الخاصة\n\n"
            "- (`.معلومات_الملصق` )\nبالرد على الملصق لعرض معلومات الحزمة\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التكرار$",
    command=("اوامر التكرار", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شـرح عـن اوامـر التـكرار\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.كرر  +عدد التكرار  +بالرد على الرسالة` )\n يقوم بتكرار النصوص والوسائط بالرد على الرسالة او الصورة \nمثال  :  ( بالرد على صورة  `.كرر 10` )\n\n"
            "- ( `.تكرار الملصق + بالرد على ملصق` )\n بالرد على الملصق ليقوم باستخراج جميع ملصقات الحزمه وارسالها\n\n"
            "- (`.مكرر  + وقت بالدقائق  + عدد  + بالرد` )\n بالرد على نص او صورة او اي شي يقوم بالتكرار  مع وقت معين .\nمثال  : بالـرد على نص ( `.مكرر 10 2` )  عندها سترسل 10 رسائل نصية ( النص الي رديت عليه ) بفاصل ثانيتين بين كل رسالة\n\n"
            "- ( `.ضع تكرار` + العدد )\nلمنع التكرار بالمجموعة الخاصة بك بالعدد الذي وضعته للعودة للوضع الطبيعي ضع 999999.\nمثال : ( `.ضع تكرار 10 )\n\n"
            "- ( `.ايقاف التكرار` )\nلأيقاف جميع التكرار ومن بينهم النشر التلقائي\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر السبام$",
    command=("اوامر السبام", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شـرح عـن اوامـر السبـام\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.سبام + كلمـة` )\nيقوم بتفصيخ احرف الكلمه وارسالها جربه بنفسك\n\n"
            "- ( `.وسبام + كلـمة` )\nكتابة الامر مع نص معين يقوم بتفصيخ الجمله كلمه كلمه وارسالها\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التنظيف$",
    command=("اوامر التنظيف", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شـرح عـن اوامـر التنظيـف\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.تنظيف + عدد الرسائل` )\nيقوم بحذف الرسائل اكتب الامر وعدد معين من الرسائل سيقوم بحذفها \n\n"
            "- ( .تنظيف  + الاضافة )\n يـجب وضع الشـارحه مع الاضافة (-)\nمثـال  :  ( `.تنظيف -ح` )  <سيقوم بحذف المتحركات في الدردشة>\n"
            "الاضافات : \n (-ب) : لحـذف الرسائل الـصوتية\n (-م): لحـذف الملفات\n (-ح): لحـذف المتحـركه\n (-ص): لحـذف الـصور\n (-غ): لحـذف الاغاني\n (-ق): لحـذف الـملصقات\n (-ر): لحـذف الـروابط \n(-ف): لحـذف الفـيديوهـات\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر المسح$",
    command=("اوامر المسح", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شـرح عـن اوامـر المسـح\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.مسح  + بالرد على النص` )\nفقط اكتب الامر بالرد على الرسالة ليقوم بحذفها \n\n"
            "- ( `.حذف رسائلي` )\nاكتب الامر في اي مكان وسيقوم بحذف جميع رسائلك في الدردشه حتى لو لم يكن لديك صلاحيات \n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الوقت والتاريخ$",
    command=("اوامر الوقت والتاريخ", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الوقت والتاريخ :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.تاريخ` )\nبالرد على الشخص او كتابة معرفه مع الامر لعرض سجل اسماء حسابه.\n\n"
            "- ( `.الوقت` )\nلعرض الوقت على شكل ملصق\n\n"
            "- ( `.وقت` )\nلعرض الوقت على شكل كتابة\n\n"
            "- ( `.مؤقت` + الوقت + النص )\nيقوم بإرسال رسالة مؤقتة و حذفها بعد وقت معين\n"
            "- مثـال : `.مؤقت 5 HELLAS `\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر كورونا$",
    command=("اوامر كورونا", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر كورونا :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.كورونا` + الدولة )\nللحصول على احصائيات فايروس كورونا\n"
            "- مثـال : `.كورونا Morocco`\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الصلاة$",
    command=("اوامر الصلاة", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الصلاة :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.صلاة` )\nاكتب الامر مع اسم محافظتك باللغه الانكليزية للحصول على اوقات الصلاة\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر مساعدة$",
    command=("اوامر مساعدة", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر المساعدة :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.موقع` + المكان )\nللحصول على مكان في الخريطه و ارساله لك\n\n"
            "- ( `.جد بكج + اسم البكج` )\nلمعرفة هل ان البكج او المكتبة موجود ام لا\n\n"
            "- ( `.صورة` )\nبالرد على الشخص للحصول على صورة حسابه الشخصية.\n\n"
            "- ( `.صوره كلها` )\nللرد على الشخص للحصول على صور حسابه الشخصية كلها.\n\n"
            "- ( `.سرعة النت` )\nارسل الامر لقياس سرعة الانترنت\n\n"
            "- ( `.حساب` )\nكتابة الامر مع معادلة رياضية و سيقوم بحلها و ارسالها لك\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="فارغ $",
    command=("فارغ ", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر، :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.معلومات` )\nارسل الامر في المجموعة لرؤية معلومات المجموعة او اكتب الامر مع معرف المجموعة\n\n"
            "- ( `.البوتات` )\nارسل الامر في المجموعة لرؤية البوتات الموجودة في المجموعة او اكتب الامر مع معرف المجموعة\n\n"
            "- ( `.المشرفين` )\nارسل الامر في المجموعة لرؤية مشرفين المجموعة او اكتب الامر مع معرف المجموعة\n\n"
            "- ( `.الاعضاء` )\nارسل الامر في المجموعة لرؤية اعضاء المجموعة او اكتب الامر مع معرف المجموعة\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الروابط$",
    command=("اوامر الروابط", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الروابط :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.دنس` + الرابط )\nلكشف نظام دومين موقع معين اكتب الامر مع الرابط\n\n"
            "- ( `.مصغر` )\nبالرد على الرابط او وضع الرابط مع الامر ليقوم بتصغيره\n\n"
            "- ( `.رابط_مخفي` )\nبالرد على الرابط لاخفاء الرابط و جعله في مسافه معينة جرب الامر بنفسك\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التحذيرات$",
    command=("اوامر التحذيرات", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر التحذيرات :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.تحذير + السبب` )\nبالرد على الشخص ليقوم بتحذيره يمكنك وضع سبب كذلك\n\n"
            "- ( `.التحذيرات` )\nبالرد على الشخص ليقوم باظهار تحذيراته.\n\n"
            "- ( `.حذف التحذيرات` )\nبالرد على الشخص لحذف تحذيراته.\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر اللستة$",
    command=("اوامر اللستة", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر اللستة :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.لستة` )\nيقوم بصنع لستة شفافة لمنشور معين\nشـرح الامـر  : \n https://t.me/k_jj_j\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الملكية$",
    command=("اوامر الملكية", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الملكية :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.ملكية` )\nيقوم بكتابة التمر في القناة التي تريد تحويلها لشخص و بكتابة الامر مع معرف الشخص\n\n"
            "لاستخدام امر نقل الملكية تحتاج لوضع TG_2STEP_VERIFICATION_CODE و معه رمز حسابك في الفارات \n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر السليب$",
    command=("اوامر السليب", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر السليب :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.سليب` + السبب)\nيقوم بادخالك في وضع غير المتصل يقوم بالرد على اي شخص برسالة يقول فيها انك غير متواجد.\n"
            "- مثـال : `.سليب نايم`\n\n"
            "- ( `.سليب_ميديا` + السبب )\nيقوم بنفس وظيفة الامر العادي غير انه يمكنك ارفاق صورة او متحركة بالرد عليها مع الامر.\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الاسم$",
    command=("اوامر الاسم", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الاسم الوقتي :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.اسم وقتي` )\nبكتابة الامر لاضافة اسم وقتي حسب منطقة التي وضعتها .\n\n"
            "- ( `.انهاء اسم وقتي` )\nلانهاء الاسم الوقتي و ارجاع الاسم الطبيعي .\n\n"
            "- ( `.الصورة الوقتية` )\n لوضع صورة تتغير مع الوقت\n\n"
            "- ( `.انهاء الصورة الوقتية` )\n لأنهاء الصورة واسترجاع الصورة الاصلية\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر البايو$",
    command=("اوامر البايو", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر البايو الوقتي :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.بايو وقتي` )\nبكتابة الامر لاضافة بايو وقتي حسب منطقة التي وضعتها .\n\n"
            "- ( `.انهاء بايو وقتي` )\nلانهاء البايو الوقتي و ارجاع البايو الطبيعي .\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الكروب الوقتي$",
    command=("اوامر الكروب الوقتي", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الكروب الوقتي :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.كروب وقتي` )\nبكتابة الامر لاضافة كروب وقتي حسب منطقة التي وضعتها .\n"
            "- ( `.انهاء كروب وقتي` )\nلانهاء الكروب الوقتي \n"
            "- ( `.كروب صورة وقتي` )\nبكتابة الامر لتشغيل الصورة الوقتية للمجموعة حسب منطقة التي وضعتها .\n"
            "- ( `.انهاء كروب صورة وقتي` )\n لأنهاء الصورة الوقتية للمجموعة.\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التشغيل$",
    command=("اوامر التشغيل", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر التشغيل :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.اعادة التشغيل` )\nلاعادة تشغيل البوت و ارجاعة الى شكله الطبيعي كما كان في الاول فقط ارسل الامر .\n\n"
            "- ( `.تحديث` )\nفقط ارسل الامر للتاكد اذا كان هناك تحديثات من مطورين السورس .\n\n"
            "- ( `.التحديثات تشغيل` )\nيستخدم هذا الامر لارسال رسالة تجريبية تلقائية لرؤية البوت اذا ما كان شغال بعد اعادة التشغيل او التحديث سيرس امر `.بنك` ولايقافه ارسل `.التحديثات ايقاف` .\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الاطفاء$",
    command=("اوامر الاطفاء", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الاطفاء :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.اطفاء` )\nلالغاء تشغيل البوت و يجب اعادة تشغيله من هيروكو .\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الكشف$",
    command=("اوامر الكشف", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الكشف :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.الايدي` )\nبالرد على الشخص او كتابة معرفه مع الامر لعرض معلوماته.\n\n"
            "- ( `.ايدي` )\nبالرد على الشخص لعرض معلوماته\n\n"
            "- ( `.كشف` )\nبالرد على الشخص لعرض معلوماته بشكل مبسط\n\n"
            "- ( `.رابط الحساب` )\nيالرد على الشخص للحصول على رابط حساب الشخص\n\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر كوكل$",
    command=("اوامر كوكل", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر كوكل :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.صور +  عدد الصور + نص` )\nللحصول على صور من متصفح كوكل بكتابة الامر وعدد الصور الحد 10 واسم النص \n"
            "- ( `.استخرج + عربي` ) \nلاستخراج النصوص من الصور التي تحتوي على كلمات عربيه\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الاذاعه$",
    command=("اوامر الاذاعه", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح اوامر الـتوجيه والأذاعـة :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.وجه + نص` )\nلعمـل اذاعـة للـنص في المجـموعـات اكتب الامر ونـصـك\n"
            "- ( `.حول + نص` )\nلعمـل اذاعـة للـنص للـ الدردشـات الخـاصة اكتب الامر ونـصـك\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="امر الصورة الذاتية$",
    command=(" امر الصورة الذاتية", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شرح امر الصورة الذاتية :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.ذاتية` )\nبالـرد عـلى الصـورة ذاتيـة الـتدمير ليـقوم بـحفظها وارسالـها لك فـي الرسائل الـمحفوظة بسرية وبـدون عـلم الـطرف الأخر \n"
            "- ( `.حفظ` )\nبالـرد عـلى الصـورة الموجوده في المجموعة ذات التقييد المحتوى ليـقوم بـحفظها وارسالـها لك فـي الرسائل الـمحفوظة بسرية وبـدون عـلم الـطرف الأخر\n"
            "( `.الذاتية تشغيل` )\nقم بكتابة الامر في أي مكان ليقوم بتفعيل الامر وحفظ الصور الذاتية التدمير تلقائياً\n"
            "( `.الذاتية تعطيل` )\nقم بكتابة الامر في مكان لتقوم بتعطيل الامر الصورة الذاتية التدمير\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التسلية$",
    command=("اوامر التسلية", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر التسلية :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه القوائم\n\n"
            "- ( `.تسلية 1` )\n- ( `.تسلية 2` )\n- ( `.تسلية 3` ) \n- ( `.تسلية 4` )\n- ( `.تسلية 5` )\n- ( `.تسلية 6` )\n- ( `.تسلية 7` )\n- ( `.تسلية 8` ) \n- ( `.فيك typing 300` ) \n- ( `.رفع ادمن` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التحشيش$",
    command=("اوامر التحشيش", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر التحشيش :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه القوائم\n\n"
            "**جميع اوامـر التحشيش تستخـدم بالـرد عـلى الشخص**\n\n"
            "- ( `.رفع تاج` )\n- ( `.رفع بكلبي` )\n- ( `.رفع مطي` )\n- ( `.رفع جلب` ) \n- ( `.رفع قرد` )\n- ( `.رفع مرتي` )\n- ( `.رفع زوجي` )\n- ( `.نسبة الانوثة` )\n- ( `.نسبة الحب` )\n- ( `.نسبة الغباء` ) \n- ( `.رفع زاحف` ) \n- ( `.رفع كحبة` ) \n- ( `.رفع فرخ` ) \n- ( `.رزله` ) \n- ( `.رفع صاك` ) \n- ( `.رفع حاته` ) \n- ( `.رفع بقره` ) \n- ( `.رفع ايجة` )\n- ( `.رفع زبال` )\n- ( `.رفع كواد` )\n- ( `.رفع ديوث` )\n- ( `.رفع مجنب` )\n- ( `.رفع مميز` )\n- ( `.رفع ادمن` )\n- ( `.رفع منشئ` )\n- ( `.رفع مالك` )\n- ( `.رفع وصخ` )\n- ( `.نسبة الكذب` )\n- ( `.نسبة الدياثه` )\n- ( `.نسبة الشذوذ` )\n- ( `.نسبة الجمال` )\n- ( `.نسبة الخيانه` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التحويل$",
    command=("اوامر التحويل", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "شـرح عـن اوامـر تـحويل الصـيغ :\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.تحويل صورة` )\nبالرد على الملصق لتحويله الى صورة\n"
            "- ( `.تحويل ملصق` )\nبالرد على الصورة لتحويلها الى الملصق\n"
            "- ( `.تحويل voice` )\nبالرد على مقطع اغنية لتحويله على شكل بصمة صوتية\n"
            "- ( `.تحويل mp3` )\nبالرد على البصمة الصوتية لتحويلها على شكل مقطع mp3\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الجهات$",
    command=("اوامر الجهات", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر الجهات والاضافة\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.ضيف + رابط مجموعة` )\nيستخدم الامر لاخذ اعضاء من مجموعة ثانية واضافتهم في مجموعتك ترسل الامر مع رابط القروب\n"
            "مثال : .ضيف https://t.me/k_jj_jSupport\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الحساب$",
    command=("اوامر الحساب", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر الحساب والقنوات والمجموعات التي تديرها\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.معلوماتي` )\n- ( `.قائمة قنواتي` )\n- ( `.قائمة كروباتي` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الترفيه$",
    command=("اوامر الترفيه", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر الترفيه\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.بلي` )\n- ( `.كت` )\n- ( `.خيروك` )\n- ( `.غنيلي` )\n- ( `.شعر` )\n- ( `.قران` )\n- ( `.البنك` )\n- ( `.فلم` )\n- ( `.ثيم` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="بصمات ميمز$",
    command=("بصمات ميمز", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة بصمات الميمز\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.تخوني` )\n- ( `.مستمرة الكلاوات` )\n- ( `.احب العراق` )\n- ( `.احبك` )\n- ( `.اخت التنيج` )\n- ( `.اذا اكمشك` )\n- ( `.اسكت` )\n- ( `.افتهمنا` )\n- ( `.اكل خرا` )\n- ( `.العراق` )\n- ( `.الكعده وياكم` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="بصمات ميمز2$",
    command=("بصمات ميمز2", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة بصمات الميمز\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.الكمر اني` )\n- ( `.اللهم لا شماته` )\n- ( `.اني مااكدر` )\n"
            "- ( `.بقولك ايه` )\n- ( `.تف على شرفك` )\n- ( `.شجلبت` )\n"
            "- ( `.شكد شفت ناس` )\n- ( `.صباح القنادر` )\n- ( `.ضحكة فيطية` )\n"
            "- ( `.طار القلب` )\n- ( `.غطيلي` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="بصمات ميمز3$",
    command=("بصمات ميمز3", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر بصمات الميمز\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.في منتصف الجبهة` )\n- ( `.لاتقتل المتعه` )\n- ( `.لا لتغلط` )\n"
            "- ( `.لا يمه لا` )\n- ( `.لحد يحجي وياي` )\n- ( `.ماادري يعني` )\n"
            "- ( `.منو انت` )\n- ( `.مو صوجكم` )\n- ( `.خوش تسولف` )\n"
            "- ( `.يع` )\n- ( `.زيج` )\n- ( `.زيج2` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="بصمات ميمز4$",
    command=("بصمات ميمز4", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر بصمات الميمز\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.يعني مااعرف` )\n- ( `.يامرحبا` )\n- ( `.منو انتة` )\n"
            "- ( `.ماتستحي` )\n- ( `.كعدت الديوث` )\n- ( `.عيب` )\n"
            "- ( `.عنعانم` )\n- ( `.طبك مرض` )\n- ( `.سييي` )\n"
            "- ( `.سبيدر مان` )\n- ( `.خاف حرام` )\n- ( `.تحيه لاختك` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="بصمات ميمز5$",
    command=("بصمات ميمز5", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر بصمات الميمز\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.امشي كحبة` )\n- ( `.امداك` )\n- ( `.الحس` )\n"
            "- ( `.افتهمنا` )\n- ( `.اطلع برا` )\n- ( `.اخت التنيج` )\n"
            "- ( `.اوني تشان` )\n- ( `.اوني تشان2` )\n- ( `.بجيت` )\n"
            "- ( `.نشاقة` )\n- ( `.لاتغلط` )\n- ( `.احب الله` )\n"
            "- ( `.روح` )\n- ( `.خبز يابس` )\n- ( `.خيار بصل` )\n"
            "- ( `.ماي ارو` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الفارات",
    command=("اوامر الفارات", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر الفارات\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.جلب + اسم الفار` ) - لجلب قيمة الفار إن وجد\n"
            "- ( `.وضع توقيت` ) - بالرد على المنطقة الزمنية\n"
            "- ( `.وضع رمز الاسم` ) - بالرد على الرمز المراد وضعه\n"
            "- ( `.وضع الكروب` ) - بالرد على اسم الكروب الذي تريد وضعهُ\n"
            "- ( `.وضع البايو` ) - بالرد على النبذا المراد وضعها\n"
            "- ( `.وضع لون وقتي` ) - بالرد على اسم اللون بالانجليزي\n"
            "- ( `.وضع الصورة` ) - بالرد على رابط تليجراف الصورة المراد وضعها\n"
            "- ( `.وضع صورة الكروب` ) - بالرد على رابط تليجراف الصورة المراد وضعها\n"
            "- ( `.وضع زخرفة الارقام` ) - بالرد على الارقام المراد وضعها\n"
            "- ( `.وضع اسم` ) - بالرد على الاسم المراد وضعه\n"
            "- ( `.وضع كروب التخزين` ) - بالرد على الايدي المحادثة المراد جعلها كروب التخزين\n"
            "- ( `.وضع كروب الحفظ` ) - بالرد على الايدي المحادثة المراد جعلها كروب الحفظ\n"
            "- ( `.وقت العراق` ) - لوضع الساعة بتوقيت العراق\n"
            "- ( `.وقت السعودية` ) - لوضع الساعة بتوقيت السعودية\n"
            "- ( `.وقت مصر` ) - لوضع الساعة بتوقيت مصر\n"
            "- ( `.وقت الاردن` ) - لوضع الساعة بتوقيت الاردن\n"
            "- ( `.وقت اليمن` ) - لوضع الساعة بتوقيت اليمن\n"
            "- ( `.وقت سوريا` ) - لوضع الساعة بتوقيت سوريا\n"
            "- تنويه: عندما تريد حذف الفار استعمل الامر (محو) بدل كلمة (وضع)\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر الوهمي",
    command=("اوامر الوهمي", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر الوهمي\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.كتابة` ) - مثال : .كتابة + عدد الثواني\n"
            "- ( `.صوتية` ) - مثال : .صوتية + عدد الثواني\n"
            "- ( `.فيد` ) - مثال : .فيد + عدد الثواني\n"
            "- ( `.لعبة` ) - مثال : .لعبة + عدد الثواني\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="بصمات انمي$",
    command=("بصمات انمي", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر بصمات الانمي\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.انمي1` )\n- ( `.انمي2` )\n- ( `.انمي3` )\n"
            "- ( `.انمي4` )\n- ( `.انمي5` )\n- ( `.انمي6` )\n"
            "- ( `.انمي7` )\n- ( `.انمي8` )\n- ( `.انمي9` )\n- ( `.انمي10` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التجميع$",
    command=("اوامر التجميع", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر التجميع\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.تجميع المليار` )\n- ( `.تجميع HELLAS ` )\n- ( `.تجميع ارشقلي` )\n"
            "- ( `.تجميع ارشقلي` )\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر التخصيص$",
    command=("اوامر التخصيص", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر التخصيص\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- قريباً . .\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )

@l313l.ar_cmd(
    pattern="اوامر وعد$",
    command=("اوامر وعد", plugin_category),
)
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "قائمة اوامر وعد\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉•★➖\n"
            "᯽︙ اختر احدى هذه الاوامر\n\n"
            "- ( `.راتب وعد` ) : عند كتابة الأمر في المجموعة، يقوم البوت بتجميع الأموال كل ١٠ دقيقة.\n"
            "- ( `.ايقاف راتب وعد` ) : يستخدم لإيقاف امر تجميع الراتب من بوت وعد.\n"
            "- ( `.سرقة وعد + ايدي الشخص` ) : يستخدم لسرقة أموال شخص آخر كل ١٠ دقيقة بواسطة إرسال ايدي المستخدم مع الامر.\n"
            "- ( `.ايقاف سرقة وعد` ) : يستخدم لإيقاف امر سرقة الأموال من الآخرين.\n"
            "- ( `.بخشيش وعد` ) : يستخدم لإعطاء بخشيش لك كل ١٠ دقيقة في المجموعة.\n"
            "- ( `.ايقاف بخشيش وعد` ) : يستخدم لإيقاف امر اخذ بخشيش من بوت وعد.\n"
            "- ( `.استثمار وعد` ) : يستخدم لاستثمار أموالك مع كل ٢٠ دقيقة.\n"
            "- ( `.ايقاف استثمار وعد` ) : يستخدم لإيقاف عملية الاستثمار بأموالك.\n"
            "★•┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉•★➖\n"
            "{CH}"
        )
